...
void DiffDriverController::sendcmd(const turtlebot_teleop::twist_hh &command)
{
    ...
    char speed[2]={0,0};//右一左二
     ...
     char cmd_str[29]={(char)0xcd,(char)0xeb,(char)0xd7,(char)0x0d, //包头和有效指令数
     (char)0x74,(char)0x00,(char)0x00,(char)0x00,(char)0x00,(char)0x00,(char)0x00,  //'t'字符,2个履带电机控制,摄像头控制
     ...
     (char)0x00,(char)0x00,(char)0x00,(char)0x00,                                   //输送带,摄像头俯仰
     (char)0x0d,(char)0x0a};  
     //0xcd 0xeb 0xd7代表包头, 0x05表示之后有5个有效数据, 0x74(t字符)表示电机命令, 紧接着两个u8字符表示右轮和左轮的状态
    ...
    speed_lin_y=command.odom_linear;       //底盘电机的前进后退
    speed_ang=command.odom_angular;        //底盘电机的旋转
    camera=command.camera_angle;           //摄像头的旋转
...
/***************摄像头**********************/
    if(camera<0)
         {
             cmd_str[9]=(char)0x42;//B 顺时针
             cmd_str[10]=-camera;
         }
         else if(camera>0)
         {
             cmd_str[9]=(char)0x46;//F  逆时针
             cmd_str[10]=camera;
         }
         else
         {
...
/*****************斜板抽屉********************/
    if(oblique_drawer<0)
         {
             cmd_str[19]=(char)0x42;//B
             cmd_str[20]=-oblique_drawer;
         }
         else if(oblique_drawer>0)
         {
             cmd_str[19]=(char)0x46;//F
             cmd_str[20]=oblique_drawer;
         }
         else
         {
...
/*****************输送带********************/
    if(belt<0)
         {
             cmd_str[23]=(char)0x42;//B
             cmd_str[24]=-belt;
         }
         else if(belt>0)
         {
             cmd_str[23]=(char)0x46;//F
             cmd_str[24]=belt;
         }
         else
         {
             cmd_str[23]=(char)0x53;//S
             cmd_str[24]=(char)0x00;
         }
...
/*****************摄像头俯仰********************/
    if(camera_tilt<0)
         {
             cmd_str[25]=(char)0x42;//B
             cmd_str[26]=-camera_tilt;
         }
         else if(camera_tilt>0)
         {
             cmd_str[25]=(char)0x46;//F
             cmd_str[26]=camera_tilt;
         }
         else
         {
             cmd_str[25]=(char)0x53;//S
             cmd_str[26]=(char)0x00;
         }
/*****************摄像头俯仰********************/
