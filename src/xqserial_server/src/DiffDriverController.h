...
public:
    ...
    DiffDriverController(double max_speed_,std::string cmd_topic_,std::string func_motors_topic_,StatusPublisher* xq_status_,CallbackAsyncSerial* cmd_serial_);
    void run();
    ...
    void sendcmd_funcControl(const turtlebot_teleop::twist_hh& command);
private:
