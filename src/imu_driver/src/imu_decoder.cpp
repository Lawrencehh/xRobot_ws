...
int main (int argc, char** argv)
{
    ...
    while(ros::ok())
    {
        if(serial_port.available()){
            ...
            mag_msg.magnetic_field.x = (float)stcMag.h[0];
            mag_msg.magnetic_field.y = (float)stcMag.h[1];
            mag_msg.magnetic_field.z = (float)stcMag.h[2];
            // mag_msg.magnetic_field_covariance.x = ((float)stcGyro.w[0]/32768*2000)/180.0*Pi;
            // mag_msg.magnetic_field_covariance.y = ((float)stcGyro.w[1]/32768*2000)/180.0*Pi;
            // mag_msg.magnetic_field_covariance.z = ((float)stcGyro.w[2]/32768*2000)/180.0*Pi;
